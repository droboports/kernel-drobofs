...
static int flashInfoFill(void)
{
	...
	if (MV_TRUE == mvIsUnitMappedToThisCpu(NOR_FLASH))
	{	
#endif
	for(i=0; i<devs; i++)
	{
	...
	{
		printk("NOR flash is not mapped to this CPU\n");
	}		
#endif
#endif
...
#ifdef CONFIG_MV_INCLUDE_SFLASH_MTD
#if defined(CONFIG_MV78200)
	if (MV_TRUE == mvIsUnitMappedToThisCpu(SPI_FLASH))
	{	
#endif
	...
	{
		printk("SPI flash is not mapped to this CPU\n");		
	}		
#endif
#endif
...
/* I am unable to figure out where exactly we are having issues with uboot and linux intercore flash
 * locking so I will just use a lock at the highest level during discovery of the part
 */
...
{	
	...
	{
		printk("!!!!!flash_map_init: ERROR: Unable to get intercore flash mutex. Continuing anyways. may cause problems.\n");
	}
