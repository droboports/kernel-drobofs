...
#if defined(CONFIG_MV78200)
int mv_is_pci_mapped_to_this_cpu(int pciIf)
{
	if (pciIf < 4) 
	{
		if (MV_FALSE == mvIsUnitMappedToThisCpu(PEX00))
			return 0;
	}
	else
	{
...
void __init mv_pci_preinit(void)
{
	...
	for (pciIf = 0; pciIf < maxif; pciIf++) 
	{
		...
		if (!mv_is_pci_mapped_to_this_cpu(pciIf))
		{
			printk("pci_preinit(): PCI-E %d not mapped to this CPU.\n", pciIf);
			continue;
		}
#endif
...
#ifdef CONFIG_MV78200
	if (!mv_is_pci_mapped_to_this_cpu(nr))
	{
		printk("mv_pci_setup(): PCI-E %d not mapped to this CPU.\n", nr);		
		return 0;
	}
#endif
...
struct pci_bus *mv_pci_scan_bus(int nr, struct pci_sys_data *sys)
{
	...
	{
#ifdef CONFIG_MV78200
		if (!mv_is_pci_mapped_to_this_cpu(scanbus+1))
		{
			printk("mv_pci_scan_bus(): PCI-E %d not mapped to this CPU.\n", 
				scanbus+1);		
			return bus;
		}
#endif
