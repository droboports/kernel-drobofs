    ...
    switch(command) {
        ...
        case COM_TX_EN:
            ...
            if(value == 0)
            {
                ...
                mv_eth_tx_en = MV_FALSE;
            }
            else
            {
                mv_eth_tx_en_deep = value;
                mv_eth_tx_en = MV_TRUE;
            }
            break;
#endif /* ETH_MV_TX_EN */
...
extern void    ethRxCoal(int port, int usec);
extern void    ethTxCoal(int port, int usec);
#if (MV_ETH_VERSION >= 4)
extern void    ethEjpModeSet(int port, int mode);
...
int run_port_com(const char *buffer) {
	...
	if( scan_count != PORT_LIST_LEN) {
    	...
    	switch(command) {
        	case COM_RX_COAL:
            		ethRxCoal(port, value);
            		break;

        	case COM_TX_COAL:
            		ethTxCoal(port, value);
        	break;

#if (MV_ETH_VERSION >= 4)
        	case COM_EJP_MODE:
            		ethEjpModeSet(port, value);
            	break;
#endif /* (MV_ETH_VERSION >= 4) */
