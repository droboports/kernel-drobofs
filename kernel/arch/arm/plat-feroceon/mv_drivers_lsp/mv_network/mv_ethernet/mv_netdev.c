...
#ifdef ETH_MV_TX_EN
MV_BOOL             mv_eth_tx_en = MV_TRUE;
int                 mv_eth_tx_en_deep = 1;
#endif /* ETH_MV_TX_EN */
...
static INLINE void  eth_pkt_info_free(mv_eth_priv *priv, MV_PKT_INFO* pPktInfo)
{
    ...
    {
        mv_eth_priv  *src_priv = MV_ETH_PRIV(mv_net_devs[(int)pPktInfo->srcIdx]);
...
static INLINE void  eth_tx_enable(mv_eth_priv *priv, int queue)
{
    ...
    ret = mvEthPortTxEnable(priv->hal_priv, queue, mv_eth_tx_en_deep);
    if(ret < 0)
    {
...
static void eth_check_link_status(struct net_device *dev, int print_flag)
{
    mv_eth_priv        *priv;
 ...
 ***********************************************************/
static u32 eth_tx_done(mv_eth_priv *priv, int queue)
{
    MV_PKT_INFO*            pPktInfo;
    u32                     count = 0;
...
static INLINE int eth_rx(struct net_device *dev, unsigned int work_to_do, int queue)
{
    ...
    while( work_done < work_to_do ) {
	    ...
	    if (nfp)
	    {
            ...
            if(pIpHdr != NULL)
            {
                out_if_index = mvFpProcess(pPktInfo->pFrags->bufVirtPtr, pIpHdr, &priv->fpStats);
                if(out_if_index != -1)
                {
...
/*
    for(rxq=MV_ETH_RX_Q_NUM, rxq>=0; rxq--)
    {
        if(priv->picr == 0)
            break;

        if(priv->picr & (ETH_CAUSE_RX_READY_MASK(rxq) | ETH_CAUSE_RX_ERROR_MASK(rxq)) )
        {
...
#ifdef ETH_MV_TX_EN
               	    if( mv_eth_tx_en && (tx_priv->tx_count[txq] > 0) )
                    {
 ...
 ***********************************************************************************/
static mv_eth_priv*     eth_priv_by_port(unsigned int port)
{
...
void    mv_eth_status_print( unsigned int port )
{
    ...
    printk("TxEnable WA - %s. deep=%d\n\n", 
            mv_eth_tx_en ? "Enabled" : "Disabled", mv_eth_tx_en_deep);    
#endif /* ETH_MV_TX_EN */
...
static int __init mv_eth_init_module( void ) 
{
	...
	if (MV_FALSE == mvIsUnitMappedToThisCpu(GIGA0+port))
	{
		printk("Giga %d is not mapped to this CPU\n", port);
		continue;
	}		
#endif
