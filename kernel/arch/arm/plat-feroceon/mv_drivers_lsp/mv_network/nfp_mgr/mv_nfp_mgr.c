...
#define NULL_BINDING_PERIOD	(5*HZ) /* arbitrary value */
...
#ifdef CONFIG_MV_ETH_NFP_NAT_SUPPORT
static FP_USER_NAT_TABLE fp_user_nat_table[2];	/* we have two tables: previous and current */
int curr_table, old_table;			/* user NAT table: 0 or 1 */
static struct timer_list null_binding_timer;
#endif /* CONFIG_MV_ETH_NFP_NAT_SUPPORT */
...
{
	if( (if_index < 0) || (if_index >= FP_MAX_INTERFACES) )
	{
		/*printk("if_index %d is OUT of RANGE\n", if_index);*/
		return 0;
	}
	if (fp_eth_devs[if_index].if_type == MV_FP_IF_INV)
		return 0;
...
#ifdef CONFIG_MV_ETH_NFP_NAT_SUPPORT
static void null_binding_timer_function(unsigned long data)
{
	/* We want to go over all the NAT rules, and set only the */
	/* null binding rules which are not already set.          */
	unsigned long   flags;
	MV_FP_NAT_RULE  *curr_nat_rule;
	int             status = 0;
	...
	curr_nat_rule = mgr_nat_rule_db.rule_chain;
	while (curr_nat_rule != NULL) {
		if (curr_nat_rule->flags == MV_FP_NULL_BINDING) {
			...
			if (status == 0) {
				curr_nat_rule->flags = MV_FP_NULL_BINDING_SET;
			}
		}
		curr_nat_rule = curr_nat_rule->next;
	}
...
int fp_mgr_init(void)
{
	...
	fp_eth_devs = kmalloc((FP_MAX_INTERFACES * sizeof(struct map_eth_devs)), GFP_ATOMIC);
	if (fp_eth_devs == NULL)
		return -ENOMEM;

	for (i = 0; i < FP_MAX_INTERFACES; i++) {
...
#ifdef CONFIG_MV_ETH_NFP_NAT_SUPPORT
	memset(&null_binding_timer, 0, sizeof(struct timer_list));
	init_timer(&null_binding_timer);
	null_binding_timer.function = null_binding_timer_function;
	null_binding_timer.data = -1;
#endif
			            ...
			            struct net_device* dev)
{
	/* sanity checks */
	if( (fp_eth_devs == NULL) || 
	    (if_index < 0) || (if_index >= FP_MAX_INTERFACES) )
		return -1;
			        ...
			        int if_index, enum nf_nat_manip_type maniptype)
{
	...
	if (	(src_ip == new_src_ip) && (dst_ip == new_dst_ip) && 
    	...
    	{
#if 0 /* Don't do anything with null binding rules */
        	if( fp_rule_find(src_ip, dst_ip) != NULL)
		    	null_binding = 1;
        	else
#endif
            		return 0;
	...
	for (curr_rule = mgr_nat_rule_db.rule_chain; curr_rule != NULL; curr_rule = curr_rule->next) {
		if (	curr_rule->srcIp == src_ip && 
			...
			curr_rule->proto == proto) {
				...
				if (null_binding) {
					curr_rule->flags = MV_FP_NULL_BINDING;
				}
				else {
					if (maniptype == IP_NAT_MANIP_DST) {
				...
				/* Now we have a full rule - we can update the NFP database       */
				/* We update null binding rules in a timer function and not here, */
				/* to allow a few seconds for actual NAT rules to override them.  */
				if (!null_binding) {
#ifdef CONFIG_MV_ETH_NFP_DUAL
					status = mvIpcChanMsgSend(mgr_to_fp_chan, MV_SERVICE_NFP_ID, 
					...
					mod_timer(&null_binding_timer, (jiffies + NULL_BINDING_PERIOD));
				}
				spin_unlock_irqrestore(&nfp_mgr_lock, flags);
				return status;
		}
	...
	if (null_binding) {
		new_rule->flags = MV_FP_NULL_BINDING;
	}
	else {
		if (maniptype == IP_NAT_MANIP_DST) {
			if ((dst_port != 0) && (dst_port != new_dst_port))
